...
const getBlockLightCursor = (pos) =>
    (getArrayPosition(pos) >>> 1) + (CHUNK_VOLUME << 1);
const getSkyLightCursor = (pos) =>
    (getArrayPosition(pos) >>> 1) + (CHUNK_VOLUME >>> 1) * 5;
...
class Chunk {
    ...
    dump() {
        ...
        let currentSkyLightIndex =
            currentBlockLightIndex + (CHUNK_VOLUME >>> 1);
        ...
        for (let y = 0; y < 16; y++) {
            outputBuffers.push(
                Chunk.packingProtocol.createPacketBuffer("section", {
                    bitsPerBlock: 13,
                    palette: [],
                    dataArray: this.packBlockData(
                        this.data.slice(
                            currentDataIndex,
                    ...
                    blockLight: this.data.slice(
                        currentBlockLightIndex,
                        currentBlockLightIndex + halfChunkBlocks
                    ),
                    skyLight: this.data.slice(
                        currentSkyLightIndex,
        ...
        outputBuffers.push(
            this.data.slice(biomeStart, biomeStart + Chunk.l * Chunk.w)
        );
    ...
    packBlockData(rawdata, bitsPerBlock) {
        ...
        let resultantBuffer = Buffer.alloc(
            ((blockCount * bitsPerBlock) >>> 3) + 4
        );
        ...
        for (let block = 0; block < blockCount; block++) {
            ...
            let reversedblockdata =
                reverseBits16(rawdata.readUInt16LE(block << 1)) >>> 3;
            ...
            let aligneddata =
                reversedblockdata << (32 - bitsPerBlock - localbit);
    ...
    load(data, bitMap = 0xffff, skyLightSent = true) {
        ...
        if (unpackeddata.length !== BUFFER_SIZE) {
            throw new Error(
                "Data buffer not correct size (was " +
                    unpackeddata.length +
                    ", expected " +
                    BUFFER_SIZE +
                    ")"
    ...
    unpackChunkData(chunk, bitMap, skyLightSent) {
        ...
        for (let y = 0; y < 16; y++) {
            ...
            if ((bitMap >> y) & 1) {
                const { size, value } = this.readSection(
                    chunk.slice(offset),
                    skyLightSent
                );

                offset += size;
                blocksAddition = this.eatPackedBlockLongs(
                    value.dataArray,
                    value.palette,
                    value.bitsPerBlock
                );
                ...
                skylightsAddition = skyLightSent
                    ? value.skyLight
                    : Buffer.alloc(skyLightSize);
            } else {
                // If a chunk is skipped, we'll just fill with existing data.
                blocksAddition = this.data.slice(
                    (y * chunkBlocks) << 1,
                    ((y + 1) * chunkBlocks) << 1
                );
                blocklightsAddition = this.data.slice(
                    blockLightStart + ((y * chunkBlocks) >>> 1),
                    blockLightStart + (((y + 1) * chunkBlocks) >>> 1)
                );
                skylightsAddition = this.data.slice(
                    skyLightStart + ((y * chunkBlocks) >>> 1),
                    skyLightStart + (((y + 1) * chunkBlocks) >>> 1)
                );
            }

            blocksAddition.copy(newBuffer, (y * chunkBlocks) << 1);
            blocklightsAddition.copy(
                newBuffer,
                blockLightStart + ((y * chunkBlocks) >>> 1)
            );
            skylightsAddition.copy(
                newBuffer,
                skyLightStart + ((y * chunkBlocks) >>> 1)
            );
        }
    ...
    readSection(section, skyLightSent) {
        try {
            return Chunk.packingProtocol.read(
                section,
                0,
                skyLightSent ? "section" : "sectionNoSkylight",
                {}
            );
        } catch (e) {
    ...
    eatPackedBlockLongs(rawBuffer, palette, bitsPerBlock) {
        ...
        for (let block = 0; block < blockCount; block++) {
            ...
            let paletteId = reverseBits(
                (datatarget << localbit) >>> (32 - bitsPerBlock),
                bitsPerBlock
            );

            // Grab the data from the palette
            let paletteData =
                palette && palette.length ? palette[paletteId] : paletteId;
...
const longToByte = [
    function(buffer, offset, typeArgs) {
        // readLongToByte
...
module.exports.loader = Chunk.pickVersion;